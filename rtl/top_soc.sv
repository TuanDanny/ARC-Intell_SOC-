...
module top_soc (
    ...
    always_ff @(posedge clk_i or negedge rst_ni_async) begin
    ...
    assign wdt_reset_hold_active = (wdt_reset_hold_cnt != '0);

    // Reset Multiplexer: External Button OR Watchdog Event
    assign combined_rst_n = rst_ni_async & ~wdt_reset_hold_active;
    ...
    // [4] TIMER [5] WDT   [6] BIST  [7] SPI bridge
    localparam NB_SLAVE = 8;
    ...
    assign start_addr[7] = `SPI_BASE_ADDR;      assign end_addr[7] = `SPI_BASE_ADDR      + 32'h0FFF;
    ...
    logic        spi_overrun; // Bridge/frontend telemetry only; DSP consumes fire-and-forget.
    logic        spi_stream_restart;

    // The bridge keeps the current testbench-friendly SPI timing, but also
    // exposes an APB control/status window so the SPI block can be configured
    // like a proper SoC peripheral later on.
    apb_spi_adc_bridge #(
        .APB_ADDR_WIDTH (12),
        .SAMPLE_WIDTH   (16),
        .CMD_WIDTH      (0),
        ...
        .clk_i          (clk_i),
        .rst_ni         (rst_no),
        .paddr_i        (s_paddr[7][11:0]),
        .pwdata_i       (s_pwdata[7]),
        .pwrite_i       (s_pwrite[7]),
        .psel_i         (s_psel[7]),
        .penable_i      (s_penable[7]),
        .prdata_o       (s_prdata[7]),
        .pready_o       (s_pready[7]),
        .pslverr_o      (s_pslverr[7]),
        ...
        .stream_restart_o (spi_stream_restart)
    );
    ...
    // Mask the critical IRQ path while BIST is intentionally driving the DSP.
    ...
    dsp_arc_detect #(
        ...
        .stream_restart_i (spi_stream_restart),
        .apb_slv          (apb_dsp_if),
    ...
    assign irq_timer_tick = timer_events_wire[3];
    ...
    safety_watchdog #(
        ...
        .wdt_reset_o(wdt_reset_req)
    );

    // --- SLAVE 6: LOGIC BIST (Self-Test) ---
    logic bist_done;
    logic [15:0] bist_sig_res;
