module apb_node #(
        ...
        input wire [NB_MASTER-1:0][APB_DATA_WIDTH-1:0]   prdata_i,
        input logic [NB_MASTER-1:0] 			 pready_i,
        input logic [NB_MASTER-1:0] 			 pslverr_i,

        // CONFIGURATION PORT
        input wire [NB_MASTER-1:0][APB_ADDR_WIDTH-1:0]   START_ADDR_i,
        input wire [NB_MASTER-1:0][APB_ADDR_WIDTH-1:0]   END_ADDR_i
    );
    ...
    always_comb begin
        ...
        // Unmapped APB access phai ket thuc giao dich, neu khong CPU/TB se doi mai.
        if (psel_i && !(|psel_o)) begin
            pready_o  = penable_i;
            pslverr_o = penable_i;
        end
