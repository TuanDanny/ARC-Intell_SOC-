/*
 * Module: logic_bist
 * Description: Built-In Self-Test (BIST) Controller for INTELLI-SAFE SoC.
 *              - Compliant with ISO 26262 LBIST (Logic BIST) requirements.
 *              - Features:
 *                  1. PRPG (Pseudo-Random Pattern Generator) using 16-bit LFSR.
 *                  2. MISR (Multiple Input Signature Register) for response compression.
 *                  3. APB v3.0 Slave Interface for software control.
 *                  4. Automatic test session control (Length, Start, Stop).
...
module logic_bist #(
    ...
    always_comb begin
        ...
        case (state)
            ...
            RUN: begin
                if (cycle_cnt >= r_test_len) next_state = COMPLETE;
